/*=============================================================================
   ...
   Ported from the 2020 `direct2d` branch. Geometry-generator path: records
   lazy geometry-gen lambdas and realizes native D2D geometries at fill/stroke
   with the correct fill mode, grouping multiple sub-geometries.
=============================================================================*/
#if !defined(ARTIST_D2D_PATH_IMPL_JUNE_7_2026)
#define ARTIST_D2D_PATH_IMPL_JUNE_7_2026
...
namespace cycfi::artist::d2d
{
   ...
   public:
      ...
      // Realize the accumulated path into a single (grouped) geometry without
      // drawing it. Caller takes ownership iff `owned` is set true (a group is
      // owned; a lone cached geometry is not). Used for clipping.
      geometry*            realize_fill(bool& owned);
   ...
   )
   {
      ...
      auto hr = get_factory().CreateStrokeStyle(
         stroke_style_properties{
            line_cap, line_cap, line_cap, join, miter_limit,
